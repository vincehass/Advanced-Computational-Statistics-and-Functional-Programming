...
dft1_matrix<-function(x,k){
  ...
  (is.complex(x) && k%%1==0) && return(Im(crossprod(x,exp(-(seq(0,length(x)-1)/length(x))*(2*pi*complex(real = 0, imaginary = 1)*k)))))print("x must be a complex vector and k an integer")
}
